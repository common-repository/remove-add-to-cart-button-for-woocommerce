...
class Remove_Add_To_Cart_Button_Woocommerce_Admin {
    ...
    public function ratcw_remove_add_to_cart_button_product_data_fields() {
        ...
        woocommerce_wp_textarea_input( array(
            ...
            'value'       => wp_kses_post( get_post_meta( get_the_ID(), 'ratcw_msg_instead_cart_button', true ) ),
            'label'       => esc_html__( 'Remove Add to Cart Message :', 'remove-add-to-cart-button-woocommerce' ),
            'desc_tip'    => true,
            'description' => esc_attr__( 'Message instead of Add to Cart for this product in frontend. If you don\'t want to show the message then nothings need to add here.', 'remove-add-to-cart-button-woocommerce' ),
        ) );
        woocommerce_wp_checkbox( array(
            'id'          => 'ratcw_hide_price',
        ...
        woocommerce_wp_textarea_input( array(
            'id'          => 'ratcw_msg_instead_product_price',
            'value'       => wp_kses_post( get_post_meta( get_the_ID(), 'ratcw_msg_instead_product_price', true ) ),
            'label'       => esc_html__( 'Hide Product Price Message :', 'remove-add-to-cart-button-woocommerce' ),
            'desc_tip'    => true,
            'description' => esc_attr__( 'Message instead of Product Price for this product in frontend. If you don\'t want to show the message then nothings need to add here.', 'remove-add-to-cart-button-woocommerce' ),
        ) );
        ?> 
			</div>
			<?php 
        if ( ratcbw_fs()->is_not_paying() ) {
            ...
            echo '<li><span class="dashicons dashicons-yes"></span> ' . esc_html__( 'Exclude products for removing the Add to Cart button and Price for a category', 'remove-add-to-cart-button-woocommerce' ) . '</li>';
            echo '<li><span class="dashicons dashicons-yes"></span> ' . esc_html__( 'Product enquiry form on product details pages', 'remove-add-to-cart-button-woocommerce' ) . '</li>';
            echo '<li><span class="dashicons dashicons-yes"></span> ' . esc_html__( 'Redirect products to third-party or affiliate sites, by assigning a custom URL for products.', 'remove-add-to-cart-button-woocommerce' ) . '</li>';
    ...
    public function ratcw_save_fields( $id, $post ) {
        ...
        $ratcw_msg_instead_cart_button = wp_kses_post( $_POST['ratcw_msg_instead_cart_button'] );
        ...
        $ratcw_msg_instead_product_price = wp_kses_post( $_POST['ratcw_msg_instead_product_price'] );
        $ratcw_msg_instead_product_price = ( $ratcw_msg_instead_product_price ? $ratcw_msg_instead_product_price : '' );
        update_post_meta( $id, 'ratcw_msg_instead_product_price', $ratcw_msg_instead_product_price );
    }
